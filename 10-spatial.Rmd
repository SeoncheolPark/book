...
#### Strict stationary와 weak stationary간의 관계

$C(\mathbf{h})=\text{Cov}(Z(\mathbf{s}+\mathbf{h}), Z(\mathbf{h}))$일 때, $C(\mathbf{s},\mathbf{s}) < \infty$인 경우에는 strict stationry이면 무조건 weak stationary가 된다. 한편 Gaussian process나 Gaussian random field인 경우는 처음 두 moment만 알면 모든 정보를 알 수 있어 weak stationary이면 strict stationary 또한 성립한다고 한다.

#### Weak stationary와 intrinsic stationary간의 관계

$$2\gamma(\mathbf{h})=2(C(\mathbf{0})-c(\mathbf{h}))$$
관계식을 살펴보면, $C$가 주어지면 $\gamma$를 정의할 수 있다. 그렇다는 얘기는 weak stationary이면 intrinsic stationary도 된다는 것이다. 그러나 $\gamma(\mathbf{h})$만 알 경우 $C(\mathbf{0})$과 $C(\mathbf{h})$를 동시에 specify하지 못한다.
