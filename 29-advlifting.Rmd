# 리프팅 스킹의 최신동향 {#advlifting}

## 적응적 리프팅 스킴(adaptive lifting scheme)

우선 두 개의 prediction operator를 먼저 소개한다.
...
프라이멀과 듀얼 리프팅 스텝에 모두 결정연산자를 사용하는 컨셉이 적응적 리프틍 스킴의 핵심이며 위 그림에 잘 나타나 있다. 그리고 이 두 결정연산자들은 $\mathbf{s}_{j+1}$과 $\mathbf{d}_{j}$에 모두 dependent되어있다는 것이 특징이다.
