...
는 rank $t$ principal component case의 reduced-rank regression coefficient matrix이다. 식 \@ref(eq:PCAcriterion2)의 최소값은 
...
여기서 적분은 periodic일 수 있다. $X(t)-\mu(t)$는 각각의 곡선들이 uncorrelated random amplitude를 갖는 직교 곡선들의 유한한 합으로 생각할 수 있다.
...
L2관점에서 fPCA를 하는 것은 eigneproblem을 푸는 것과 같다고 한다. FPCA를 하는 것에는 다음과 같은 방법들이 있다.

1. **Discretizing the functions**: equally spcaed grid point $t_{1},\ldots , t_{m}$을 생각하고 covariance matrix  $\mathbf{V}^{*}$를 $\mathbf{V}(t_{i},t_{j})$로 나타낸 다음 다변량 PCA를 $\mathbf{V}^{*}$에 적합, smoothes PC를 얻기 위해 interpolation을 추가로 한다.

2. **Basis expansion of the functions**: $y_{i}(t)=\sum_{j=1}^{K}c_{ij}\phi_{k}(t)$라고 호자. $\Phi$를 orthonormal basis의 집합이라고 하자. 그리고 $\mathbf{C}$를 $(n\time K)$ coefficient matrix라고 하자. 그러면 $\mathbf{Y}=\mathbf{C}\Phi$이고 표본 공분산 행렬은 $\mathbf{V}=\frac{1}{n}\Phi^{T}\mathbf{C}^{T}\mathbf{C}\Phi$로 쓸 수 있다. 이를 이용하는 방법은 패키지 `FDA`에서 구현되어 있다.
