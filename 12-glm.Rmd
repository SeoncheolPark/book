...
[@Yee2015]에 따르면 **B-스플라인(B-spline)**을 쓰는 이유는 주로 이것이 좋은 수치적 성질을 갖고 있기 때문이라고 한다(minimal support). Minimal support의 뜻은 $B_{s,q}(x)>0$ for $x$ belonging to the smallest overall region defined by knots 라고 한다. Minimal support를 갖고 있으면, neighbourhood의 개념이 계산적으로 more stable해진다. 왜냐하면 B-spline의 계수는 the amount of overlap is minimal과 관련이 있기 때문이다.

[@Wasserman2006]에서는 B-spline이 compact support를 갖으므로 calculation speed-up을 할 수 있다고 설명하고 있다.
