...
실수값을 갖는 확률변수 $Y$가 분포함수 $F$를 따른다고 할 때, $Y$의 $\tau$번째 분위수는
$$Q_{\tau}(Y)=F_{Y}^{-1}(\tau)=\inf\{y|F(y)\geq \tau\}$$
로 정의된다.
...
참고로 $Q_{\tau}(Y)$는 감소하지 않는 함수이다. 즉
$$Q_{\tau_{1}}(Y) \leq Q_{\tau_{2}}(Y) \qquad{ for }\tau_{1} < \tau_{2}$$
이다.
...
$Y$를 반응변수, $\mathbf{X}$를 $p$차원 예측변수라고 하자. 이 때 $\mathbf{X}=\mathbf{x}$로 주어졌을 때 $Y$의 조건부 누적분포함수는 $F_{Y}(y|\mathbf{X}=\mathbf{x})=P(Y\leq y|\mathbf{X}=\mathbf{x})$라고 하자. 이 때 $Y$의 $\tau$\textbf{번째 조건부 분위수 (}$\tau$\textbf{-th conditional quantile)}는
