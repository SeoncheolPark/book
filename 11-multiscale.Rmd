...
다음과 같은 자료 $\bm{y}=(1,1,7,9,2,8,8,6)$을 다중척도 변환한 결과는 $(42,6,14,4,0,2,6,-2)$였다. 이 벡터를 $\bm{d}$라 하자. 그러면
$$\| \bm{y} \|^{2}=\sum_{i=1}^{8}y_{i}^{2}=219, \| \bm{d} \|^{2}=\sum_{i=1}^{8}d_{i}^{2}=2056$$
이다. 그러나 때때로 우리는 $\| \bm{y} \|^{2}=\| \bm{d} \|^{2}$가 되도록 만들고 싶어한다. 이를 해결해 줄 수 있는 것이 **이산 Haar 웨이블릿 변환(discrete Haar wavelet transform)**이다.
